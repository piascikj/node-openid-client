...
function authorizationParams(params) {
  ...
  const authParams = _.chain(params)
    .defaults({
...
<body onload="javascript:document.forms[0].submit()">
  ...
  authorizationCallback(redirectUri, parameters, checks) {
    console.log('In authorizationCallback');
    ...
    if (params.error) {
      console.log(`Rejecting with an OpenIdConnectError: ${JSON.stringify(params)}`);
    ...
    if (params.code) {
      console.log('In params.code if block of authorizationCallback');
      const grantCall = () =>
        this.grant({
          ...
          .then(tokenset => {
            console.log(`After decryptIdToken and validateIdToken`);
  ...
  decryptIdToken(token, use) {
    ...
    const keystoreOrSecret = expectedAlg.match(/^(RSA|ECDH)/)
      ? Promise.resolve(instance(this).keystore)
      : this.joseSecret(expectedAlg);

    return keystoreOrSecret.then(keyOrStore =>
      jose.JWE.createDecrypt(keyOrStore).decrypt(idToken).then(result => {
  ...
  validateIdToken(tokenSet, nonce, returnedBy, maxAge) {
    ...
    return (header.alg.startsWith('HS') ? this.joseSecret() : this.issuer.key(header))
      .then(key =>
        jose.JWS.createVerify(key).verify(idToken).catch(() => {
  ...
  refresh(refreshToken) {
    ...
    return this.grant({
      ...
      return this.decryptIdToken(tokenset, 'id_token').then(() => this.validateIdToken(tokenset, null, 'token', null));
    });
  }
  ...
  userinfo(accessToken, options) {
    ...
    return this.httpClient
      [verb](issuer.userinfo_endpoint, issuer.httpOptions(httpOptions))
      ...
      .then(response => {
        if (JWT_CONTENT.exec(response.headers['content-type'])) {
          return Promise.resolve(response.body).then(jwt => this.decryptIdToken(jwt, 'userinfo')).then(jwt => {
            ...
            return this.validateIdToken(jwt, null, 'userinfo', null).then(valid =>
              JSON.parse(base64url.decode(valid.split('.')[1]))
            );
          });
        }
   ...
   */
  grant(body) {
    console.log(`Inside grant(body) : ${JSON.stringify(body)}`);
    ...
    return this.authenticatedPost('token', { body: _.omitBy(body, _.isUndefined) })
      .then(response => {
        console.log(`Inside grant response : ${JSON.stringify(response)}`);
      })
      .then(expectResponse(200))
  ...
  revoke(token, hint) {
    ...
    return this.authenticatedPost('revocation', { body }).then(response => {
      if (response.body) {
        return JSON.parse(response.body);
      }
      return {};
  ...
  fetchDistributedClaims(claims, accessTokens) {
    ...
    return Promise.all(
      _.map(distributedSources, (def, sourceName) => {
        const opts = {
          headers: { Authorization: bearer(def.access_token || tokens[sourceName]) }
        };

        return this.httpClient
          .get(def.endpoint, this.issuer.httpOptions(opts))
  ...
  unpackAggregatedClaims(claims) {
    ...
    return Promise.all(
      _.map(aggregatedSources, (def, sourceName) => {
  ...
  authFor(endpoint) {
    switch (this[`${endpoint}_endpoint_auth_method`] || this.token_endpoint_auth_method) {
      ...
      case 'client_secret_jwt': {
        const timestamp = now();
        return this.createSign(endpoint).then(sign =>
          sign
            .update(
              JSON.stringify({
                iat: timestamp,
    ...
    return this.issuer
      ...
      .then(key => {
        return signed.then(cleartext => {
          return jose.JWE.createEncrypt({ format, fields }, { key }).update(cleartext).final();
        });
      });
...
Object.defineProperty(Client.prototype, 'grantAuth', {
  get: util.deprecate(
    /* istanbul ignore next */ function grantAuth() {
