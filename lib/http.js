...
module.exports.get = function get(url, options) {
  ...
  return got.get(url, options).then(response => console.log(`GOT GET RESPONSE: ${response.body}`));
};

module.exports.post = function post(url, options) {
  console.log(`POST REQUEST: ${url} ${JSON.stringify(options)}`);
  return got.post(url, options).then(response => console.log(`GOT GET RESPONSE: ${response.body}`));
};
